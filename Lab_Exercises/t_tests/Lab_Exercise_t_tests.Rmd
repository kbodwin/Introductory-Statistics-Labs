...
alt1 <- reactive(switch(input$alt_hypo_1, "two.sided" = "not equal to", "greater than" = "greater", "less" = "less than"))
...
test2 <- reactive(t.test(Age ~ Sex, data = titanic, mu = 30, alternative = "less"))
...
output$check_2.4 <- renderText(
  if(input$a_2.4 == ""){
     ""
  }else if(round(as.numeric(input$a_2.4), 3) == round(test()$p.value, 3)){
      correct
  }else{
    incorrect
  }
